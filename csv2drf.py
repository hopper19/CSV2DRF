...
class CSV2DRFConverter:
    def __init__(self, inputdir, outputdir, uuid_str=None):
...
def process_file(file_path):
    return pd.read_csv(
        ...
        header=None,
        converters={
            col: (lambda x: np.array(int(x, 16)).astype(np.int16)) for col in range(3)
        },
    ).to_numpy()
...
def main():
    ...
    for date in args.dates:
        ...
        ok, channel_dir, start_global_index = create_drf_dataset(
            data_files, output_dir, config["global"], metadata, start_time, uuid_str
        )
        print("create_drf_dataset returned", ok, file=sys.stderr)
        if not ok:
            raise DatasetCreationError()

        ok = create_drf_metadata(
            channel_dir, config, metadata, start_global_index, uuid_str
        )
